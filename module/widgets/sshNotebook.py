...
FNB_OPEN_BUTTON = 131072
FNB_MENU_BUTTON = 262144
FNB_ADD_BUTTON = 524288
...
class TabNavigatorWindow(wx.Dialog):
    ...
    def __init__(self, parent=None, icon=None):
        ...
        self._listBox = wx.ListBox(self, wx.ID_ANY, wx.DefaultPosition, wx.Size(200, 150), [],
...
class FNBRenderer(object):
    ...
    def GetRightButtonPos(self, pageContainer):
        ...
        return clientWidth - 38 -16

    def GetAddButtonPos(self, pageContainer):
        pc = pageContainer
        rect = pc.GetClientRect()
        clientWidth = rect.width

        # lamark: -20 menu_btn.
        return clientWidth - 54 - 20 - 20
    ...
    def DrawOpenButton(self, pageContainer, dc):
        pc = pageContainer

        # Set the bitmap according to the button status
        if pc._OpenButonStatus == FNB_BTN_HOVER:
            openBmp = wx.Bitmap('bitmaps/open_hover.png', wx.BITMAP_TYPE_PNG)
        else:
            openBmp = wx.Bitmap('bitmaps/open.png', wx.BITMAP_TYPE_PNG)

        openBmp.SetMask(wx.Mask(openBmp, MASK_COLOUR))

        # Erase old bitmap
        posx = 6
        self.DrawArrowAccordingToState(dc, pc, wx.Rect(posx,4, 16, 16))

        # Draw the new bitmap
        dc.DrawBitmap(openBmp, posx, 4, True)

    def DrawMenuButton(self, pageContainer,dc):
        pc = pageContainer

        # Set the bitmap according to the button status
        if pc._MenuButonStatus == FNB_BTN_HOVER:
            menuBmp = wx.Bitmap('bitmaps/ssh_menu_hover.png', wx.BITMAP_TYPE_PNG)
        else:
            menuBmp = wx.Bitmap('bitmaps/ssh_menu.png', wx.BITMAP_TYPE_PNG)

        menuBmp.SetMask(wx.Mask(menuBmp, MASK_COLOUR))

        # Erase old bitmap
        rect = pc.GetClientRect()
        posx = rect.width - 20
        self.DrawArrowAccordingToState(dc, pc, wx.Rect(posx, 4, 20, 20))

        # Draw the new bitmap
        dc.DrawBitmap(menuBmp, posx, 4, True)

    def DrawAddButton(self, pageContainer, dc):
        pc = pageContainer
        if pc._AddButonStatus == FNB_BTN_HOVER:
            addBmp = wx.Bitmap('bitmaps/add_console_hover.png', wx.BITMAP_TYPE_PNG)
        else:
            addBmp = wx.Bitmap('bitmaps/add_console.png', wx.BITMAP_TYPE_PNG)

        addBmp.SetMask(wx.Mask(addBmp, MASK_COLOUR))

        self.DrawArrowAccordingToState(dc, pc, wx.Rect(pc.btnAddPosX, 4, 20, 20))

        # Draw the new bitmap
        dc.DrawBitmap(addBmp, pc.btnAddPosX, 4, True)
    ...
    def DrawTabs(self, pageContainer, dc):
        ...
        :param `dc`: an instance of :class:`wx.DC`.
        """
        # self.DrawOpenButton(pageContainer,dc)
        pc = pageContainer
        ...
        if pc.HasAGWFlag(FNB_OPEN_BUTTON):
            posx = pc._pParent.GetPadding() + 23
        else:
        ...
        for i in range(pc._nFrom, len(pc._pagesInfoVec)):
            ...
            # lzmark:+25 add_btn
            if posx + tabWidth + self.GetButtonsAreaLength(pc) +25 >= clientWidth:
                self.DrawLeftArrow(pc, dc)
                self.DrawRightArrow(pc, dc)
                break
            ...
            self.DrawTab(pc, dc, posx+1, i, tabWidth, tabHeight, pc._nTabXButtonStatus)
        ...
        self.DrawOpenButton(pc, dc)
        self.DrawMenuButton(pc, dc)
        self.DrawAddButton(pc,dc)
...
class FNBRendererDefault(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        tabPoints[1].x = int(
            posx + (tabHeight - 2) * math.tan(float(pc._pagesInfoVec[tabIdx].GetTabAngle()) / 180.0 * math.pi))
        tabPoints[1].y = \
        (pc.HasAGWFlag(FNB_BOTTOM) and [tabHeight - (VERTICAL_BORDER_PADDING + 2)] or [(VERTICAL_BORDER_PADDING + 2)])[
            0]

        tabPoints[2].x = tabPoints[1].x + 2
        tabPoints[2].y = \
        (pc.HasAGWFlag(FNB_BOTTOM) and [tabHeight - VERTICAL_BORDER_PADDING] or [VERTICAL_BORDER_PADDING])[0]

        tabPoints[3].x = int(posx + tabWidth - (tabHeight - 2) * math.tan(
            float(pc._pagesInfoVec[tabIdx].GetTabAngle()) / 180.0 * math.pi)) - 2
        tabPoints[3].y = \
        (pc.HasAGWFlag(FNB_BOTTOM) and [tabHeight - VERTICAL_BORDER_PADDING] or [VERTICAL_BORDER_PADDING])[0]
        ...
        tabPoints[5].x = int(tabPoints[4].x + (tabHeight - 2) * math.tan(
            float(pc._pagesInfoVec[tabIdx].GetTabAngle()) / 180.0 * math.pi))
...
class FNBRendererVC71(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        dc.SetPen((tabIdx == pc.GetSelection() and [wx.Pen(wx.SystemSettings.GetColour(wx.SYS_COLOUR_3DFACE))] or [
            borderPen])[0])
        dc.SetBrush((tabIdx == pc.GetSelection() and [wx.Brush(wx.SystemSettings.GetColour(wx.SYS_COLOUR_3DFACE))] or [
        ...
        if tabIdx == pc.GetSelection():
            ...
            blackLineY1 = \
            (pc.HasAGWFlag(FNB_BOTTOM) and [VERTICAL_BORDER_PADDING + 2] or [VERTICAL_BORDER_PADDING + 1])[0]
...
class FNBRendererFancy(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        pen = (tabIdx == pc.GetSelection() and [wx.Pen(pc._pParent.GetBorderColour())] or [
            wx.Pen(wx.SystemSettings.GetColour(wx.SYS_COLOUR_3DFACE))])[0]
        ...
        if tabIdx == pc.GetSelection():
            ...
            col2 = (pc.HasAGWFlag(FNB_BOTTOM) and [pc._pParent.GetGradientColourTo()] or [
                pc._pParent.GetGradientColourFrom()])[0]
            col1 = (pc.HasAGWFlag(FNB_BOTTOM) and [pc._pParent.GetGradientColourFrom()] or [
...
class FNBRendererVC8(FNBRenderer):
    ...
    def DrawTab(self, pageContainer, dc, posx, tabIdx, tabWidth, tabHeight, btnStatus):
        ...
        dc.SetPen(wx.Pen((tabIdx == pc.GetSelection() and [wx.SystemSettings.GetColour(wx.SYS_COLOUR_BTNSHADOW)] or [
            pc._colourBorder])[0]))
        dc.DrawPolygon(tabPoints)
    ...
    def FillVC8GradientColour(self, pageContainer, dc, tabPoints, bSelectedTab, tabIdx):
        ...
        while 1:
            ...
            dc.SetPen(wx.Pen(
                (bSelectedTab and [wx.SystemSettings.GetColour(wx.SYS_COLOUR_BTNSHADOW)] or [pc._colourBorder])[0]))
...
class FlatNotebook(wx.Panel):
    ...
    def Init(self):
        ...
        self._tabsizer = wx.BoxSizer(wx.HORIZONTAL)
        self._mainSizer.Insert(0, self._tabsizer, 0, wx.EXPAND)
        self._tabsizer.Add(self._pages, 1, wx.EXPAND)
...
class PageContainer(wx.Panel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition,
        ...
        self._OpenButonStatus = FNB_BTN_NONE
        self._MenuButonStatus = FNB_BTN_NONE
        self._AddButonStatus = FNB_BTN_NONE
    ...
    def OnLeftUp(self, event):
        ...
        elif where == FNB_ADD_BUTTON:
            self.Parent.Parent.add_default_page()
    ...
    def HitTest(self, pt):
        ...
        tabIdx = -1
        # if OPEN_BUTTON
        rect = wx.Rect(6, 4, 20, 20)
        if rect.Contains(pt):
            return FNB_OPEN_BUTTON, tabIdx

        # if MENU_BUTTON
        rect = wx.Rect(fullrect.width-20, 4, 20, 20)
        if rect.Contains(pt):
            return FNB_MENU_BUTTON, tabIdx

        # if ADD_BUTTON
        rect = wx.Rect(self.btnAddPosX, 4, 20, 20)
        if rect.Contains(pt):
            return FNB_ADD_BUTTON, tabIdx
    ...
    def OnMouseMove(self, event):
        ...
        openButtonStatus = self._OpenButonStatus
        self._OpenButonStatus = FNB_BTN_NONE
        menuButtonStatus = self._MenuButonStatus
        self._MenuButonStatus = FNB_BTN_NONE
        addButtonStatus = self._AddButonStatus
        self._AddButonStatus = FNB_BTN_NONE
        ...
        if where == FNB_X:
            ...
            if event.LeftIsDown():
                ...
                self._nArrowDownButtonStatus = \
                (self._nLeftClickZone == FNB_DROP_DOWN_ARROW and [FNB_BTN_PRESSED] or [FNB_BTN_NONE])[0]
            ...
            if event.LeftIsDown():
                ...
                (self._nLeftClickZone == FNB_TAB_X and [FNB_BTN_PRESSED] or [FNB_BTN_NONE])[0]
            ...
            if event.LeftIsDown():
                ...
                self._nRightButtonStatus = \
                (self._nLeftClickZone == FNB_RIGHT_ARROW and [FNB_BTN_PRESSED] or [FNB_BTN_NONE])[0]
            ...
            if event.LeftIsDown():
                ...
                self._nLeftButtonStatus = \
                (self._nLeftClickZone == FNB_LEFT_ARROW and [FNB_BTN_PRESSED] or [FNB_BTN_NONE])[0]
            ...
            self._OpenButonStatus = FNB_BTN_HOVER

        elif where == FNB_MENU_BUTTON:
            self._MenuButonStatus = FNB_BTN_HOVER

        elif where == FNB_ADD_BUTTON:
            self._AddButonStatus = FNB_BTN_HOVER
        ...
        bRedrawOpen = self._OpenButonStatus != openButtonStatus
        bRedrawMenu = self._MenuButonStatus != menuButtonStatus
        bRedrawAdd = self._AddButonStatus != addButtonStatus

        render = self._mgr.GetRenderer(agwStyle)

        if (bRedrawX or bRedrawRight or bRedrawLeft or bRedrawTabX or bRedrawDropArrow or bRedrawTabs
                or bRedrawOpen or bRedrawMenu or bRedrawAdd):
            ...
            if bRedrawOpen:
                render.DrawOpenButton(self,dc)

            if bRedrawMenu:
                render.DrawMenuButton(self,dc)

            if bRedrawAdd:
                render.DrawAddButton(self, dc)
